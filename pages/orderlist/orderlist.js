...
Page({
  ...
  navtor: function(e) {   //订单跳转    商品详情改正==>  订单详情
   
    // var secondtype = e.currentTarget.dataset.secondtype;
    // var productId = JSON.stringify(e.currentTarget.dataset.product);
    // if (secondtype == 16){
    //   wx.navigateTo({
    //     url: '/integratShop/commodityDetails/commodityDetails?pid=' + productId
    //   })
    // }else{
    //   wx.navigateTo({
    //     url: '../productdetail/productdetail?pid=' + productId
    //   })
    ...
    if(type == 0){
      wx.navigateTo({
        url: '../orderdetail/orderdetail?on='+ e.currentTarget.dataset.ordernum + "&tp=" + e.currentTarget.dataset.ordertype

      })
    } else if (type == 2){
      wx.navigateTo({
        url: '../favorableorderdetail/favorableorderdetail ? on =' + e.currentTarget.dataset.ordernum
      })
