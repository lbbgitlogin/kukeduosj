...
Page({
  ...
  onLoad: function(options) {
    ...
    $.xsr($.makeUrl(integraiAPI.GetPointProductBaseInfo, val), function(data) {
      console.log("商品信息",data)
      if (!$.isNull(data.Info)) {
        if (data.Info == 2) {
          $.gotopage("../../server/productdetail/productdetail?pid=" + options.pid);
        } else {
  ...
  InitProduct: function() { //初始化商品
    ...
    $.xsr($.makeUrl(integraiAPI.GetPointProductBaseInfo, valInfo), function(data) {
      if (data.Code == 0) {
    ...
    $.xsr($.makeUrl(integraiAPI.GetPointProductCommentInfo, valser), function(data) {
      if (data.Code == 0) {
        that.setData({
  ...
  getSkuPrice: function(specIdStr) { //获取商品价格信息
    ...
    var that = this;
    $.xsr($.makeUrl(integraiAPI.GetPointProductSKUAndPriceList, valprice), function(data) {
      ...
      that.setData({
        speclist: Speclist,
        intNum: data.Info.SalePricePoint
      })
  ...
  updatePrice: function(sparray) {
    ...
    var pointVo = this.data.ProductPoint;
    var pointList = this.data.ProductPoint.PriceList;

    for (var i in pointList) {
      var flag = true;
      for (var j in pointList[i].SpecIdList) {

        var speFlag = false;
        for (var x in sparray) {
          if (pointList[i].SpecIdList[j] == sparray[x]) {
            speFlag = true;
            break
          }
        }
      ...
      if (flag) {
        pointVo.ProductSkuId = pointList[i].ProductSkuId;
        pointVo.SalePricePoint = pointList[i].SalePricePoint;

        if (pointList[i].EventId > 0) {
          pointVo.ShowPrice = pointList[i].PreferentialPrice
        } else {
          pointVo.ShowPrice = pointList[i].ShowPrice
        }
        pointVo.MarketPrice = pointList[i].MarketPrice;
        pointVo.Stock = pointList[i].Stock;
        pointVo.EventId = pointList[i].EventId;
        break
      }
    }
    ...
    for (var i in pointList) {
      ...
      if (flag) {
        //修改规格积分
        pointVo.ProductSkuId = pointList[i].ProductSkuId;
        pointVo.SalePrice = pointList[i].SalePricePoint;

        if (pointList[i].EventId > 0) {
          pointVo.ShowPrice = pointList[i].PreferentialPrice
        } else {
          pointVo.ShowPrice = pointList[i].SalePricePoint
        }
        pointVo.MarketPrice = pointList[i].MarketPrice;
        pointVo.Stock = pointList[i].Stock;
        pointVo.EventId = pointList[i].EventId;
  ...
  add: function() { //加数量
    // if (this.data.numval < this.data.EPointLimitTotal){
    //   this.unifiedNum(1);
    // }else{
    //   $.alert("你已超出限购数量")
    // }
  ...
  writenum: function(even) { //失去焦点时
    var that = this;
    if (even.detail.value <= that.data.EPointLimitTotal){
  ...
  onShareAppMessage: function() {
    return {
      ...
      path: '/integratShop/commodityDetails/commodityDetails?pid=' + this.data.proId + "&uid=" + app.globalData.UserInfo.Id
    }
  },
  buynow: function(e) { //立即购买
    ...
    wx.navigateTo({
      url: "/integratShop/IntegralOrder/IntegralOrder?spid=" + encodeURIComponent(JSON.stringify(val)) + "&orderType=" + "0" //特殊字符转译
    });
    this.setData({
      selectct: 0
    });
